...
def main():
    print("🔍 Sprawdzam ostatnie notowania M1...\n")
    ...
    for symbol in sorted(os.listdir(BASE_DIR)):
        ...
        m1_path = os.path.join(symbol_path, TIMEFRAME)
        if not os.path.exists(m1_path):
            continue
            
        csv_file = f"{symbol}_{TIMEFRAME}.csv"
        csv_path = os.path.join(m1_path, csv_file)
    ...
    if results:
        ...
        filename = f'm1_last_quotes_{timestamp}.csv'
        saved_file = save_to_csv(df, filename)
        if saved_file:
            print(f"\n💾 Wyniki zapisano do pliku: {saved_file}")
    
    print("\n✅ Zakończono sprawdzanie notowań M1")
